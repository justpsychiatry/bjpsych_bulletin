...
-   [Method](#S1){#toc-S1}
    ...
    -   [Ethical considerations](#S4){#toc-S4}
    -   [Analysis](#S5){#toc-S5}
        -   [Stage 1](#S6){#toc-S6}
        -   [Stage 2](#S7){#toc-S7}
        -   [Stage 3](#S8){#toc-S8}
        -   [Stage 4](#S9){#toc-S9}
-   [Results](#S10){#toc-S10}
    -   [Results of thematic analysis](#S11){#toc-S11}
        -   [Theme 1: unprepared for difference](#S12){#toc-S12}
        -   [Theme 2: need for support](#S13){#toc-S13}
        -   [Theme 3: value of the group](#S14){#toc-S14}
        -   [Theme 4: judgement in supervision](#S15){#toc-S15}
    -   [Theme 5: group structure](#S16){#toc-S16}
-   [Discussion](#S17){#toc-S17}
    -   [Strengths and limitations](#S18){#toc-S18}
...
psychopharmacology.^[@R1]^ They may also have to confront substantial
...
worked with.^[@R2]^ Prompted by these findings, trainees on the Bristol
...
their experiences.^[@R3]^ Although one-on-one interviews were an
...
detail in Rabiee was followed.^[@R5]^
...
reviewed as suggested by Rabiee,^[@R5]^ as individuals could not be
...
by a process of abstraction as described by Fade.^[@R6]^ A requirement
for reflexivity was acknowledged from the outset. Thus, researchers\'
...
## Results of thematic analysis {#S11}
...
psychiatry.^[@R1],[@R2]^ One strong theme that emerged from our results
...
disorder, thus treatment of any mental illness may restore the
patient\'s previous level of functioning and quality of life, but no
further. Despite epilepsy management being a common role for the
intellectual disability psychiatrist,^[@R8]^ our findings suggest that
many trainees felt apprehensive about their skills and confidence in
this area.
...
off-putting to foundation doctors,^[@R9]^ but few studies explore
feelings of isolation among psychiatry trainees^[@R2]^ It has been noted
previously that individuals who work with people with disabilities can
feel stigmatised and isolated.^[@R10]--[@R12]^ Stigma by association is
the process by which relatives, support staff, friends and associates
feel stigmatised owing to their contact with a stigmatised
group.^[@R13],[@R14]^ This may also affect trainees working in this area
...
absent in previous work,^[@R2]^ but are important to note to reassure
future recruits in the specialty. In particular, the trainees mentioned
the term 'rewarding', a varied and complex concept.^[@R15]^ The view
that community-based intellectual disability psychiatry would be
rewarding was predicted some 30 years ago,^[@R16]^ although this is the
first study as far as we are aware that affirms this view. Intellectual
disability requires a particularly holistic approach, often not
dissimilar to the approach of general practitioners (GPs). While there
is an absence of published surveys or qualitative studies on what
...
GPs.^[@R17]^ However, we are unaware of similar studies among
psychiatrists.^[@R18]^
...
find it useful.^[@R19]^ The Royal College of Psychiatrists recommends
...
matters'.^[@R20]^ The hierarchical nature of consultant supervision is
both valued by trainees^[@R21]^ and necessary for valid competency
...
boundary breach in supervision.^[@R22]^ The trainee support group is set
up without hierarchy and this may have contributed to reducing
...
should include this.^[@R20]^ This study adds to previous work suggesting
some mismatch between intent and what trainees feel comfortable
discussing^[@R23]^ This space for honesty is a value of the group but it
would be a concern if the availability of the trainee support group and
its support acted as a colluder or barrier to honesty in consultant
supervision.
...
and existential factors.^[@R24]^ That senior trainees stayed in the
...
the group may provide additional support.^[@R25]^ It also provides
senior opinions, which have been found to be supportive when shared in
other contexts.^[@R26]^ A common concern when leaderless groups are used
for supervision is a loss of focus on the task and drifting into support
and advice-giving.^[@R27],[@R28]^ As the model presented here is
